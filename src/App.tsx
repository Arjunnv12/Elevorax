...
export default function App() {
  ...
  const [selectedCourse, setSelectedCourse] = useState('Java + Python Power Bundle');
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      scrollTimeline
        .fromTo('#boy-character-wrapper',
          { x: 0, y: 0 },
          { x: 15, y: 0, duration: 0.15, ease: 'none' }
        )
        .to('#boy-character-wrapper',
          { x: 215, y: -310, duration: 0.7, ease: 'none' }
        )
        .to('#boy-character-wrapper',
          { x: 230, y: -310, duration: 0.15, ease: 'none' }
        );
    });
  ...
  const isGreenGlow = progress >= 0.80;
  ...
  return (
    <div className="min-h-screen app-container text-gray-800 font-space antialiased selection:bg-slate-800 selection:text-white">
      ...
      <section id="home" className="relative pt-[72px] pb-16" style={{ overflowX: 'clip', overflowY: 'visible' }}>
        ...
        <div id="home-scroll-container" className="max-w-7xl mx-auto w-full px-6 flex flex-col md:flex-row gap-16 md:gap-34 items-start relative z-10">          {/* LEFT HALF (text/intro & levels) */}
